...
const TabPanel: FC<{ value: string; index: string; children: ReactNode }> = ({value, index, children}) => {
  return (
    <TabPanelStyled
      role="tabpanel"
      hidden={value !== index}
...
const FilterContent: FC<{
  ...
  const handleChange = (event: ChangeEvent<unknown>, newValue: string) => seCurrentTab(newValue);
  
  const handleSelect: (changedNodes: IData[] | undefined) => void = (changedNodes) => {
    ...
    if (!multipleSelection) {
      setSelected(selectedValues.includes(changedValues[0]) ? [] : changedNodes);
    } else if (changedValues.length === 0) {
      setSelected(changedNodes);
    } else if (changedNodes.every(({value}) => selectedValues.includes(value))) {
      // unselect a node
  ...
  return (
    <TreeStyled isContentOpened={isOpened} width={width} {...otherProps}>
      <div className="wrapper">
        <SelectionHeading style={{backgroundColor: bgColor}} onClick={() => setOpened(true)}>
          {!isOpened && <div className="icon">{icon}</div>}
          <Summary
            label={label}
            isOpened={isOpened}
            selected={selected}
          ...
          <AppBar position="sticky">
            ...
            >
              {dataGroup.map((d, index) => (
                <Tab value={`tab${index}`} key={`tab${index}`} label={d.label}/>
              ))}
            </Tabs>
          </AppBar>
          {dataGroup.map((d, index) => (
            <TabPanel value={currentTab} index={`tab${index}`} key={`tab${index}`}>
