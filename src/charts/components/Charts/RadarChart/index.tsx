...
export const RadarChart = ({ width, height, seriesData, conf }: RadarChartProps) => {
  ...
  yScale.range([0, height / 2 - (conf?.padding || 0)]);
    ...
    <div>
      <svg width={width} height={height} style={{ overflow: 'visible' }} ref={containerRef}
        ...
        <Group top={height / 2} left={width / 2}>
          <GridAngle
            scale={radialScale}
            outerRadius={height / 2 - (conf?.padding || 0)}
          ...
          <AxisLeft
            top={-height / 2 + (conf?.padding || 0)}
            scale={reverseYScale}
            numTicks={5}
            tickStroke="none"
            hideTicks
            stroke="transparent"
            tickLabelProps={() => (conf?.axes?.topVertical?.style ? {...conf?.axes?.topVertical?.style, stroke:  "#fff"} : {})}
